...
export function recordAudio(filename, { maxSilenceMs, silenceVolumeThreshold } = {}) {
    return new Promise((resolve, reject) => {
        let totalSilence = 0;

        console.info(chalk.bold.red('RECORDING'));
        console.info(chalk.dim('max silence ms'), maxSilenceMs);
        console.info(chalk.dim('vol threshold'), silenceVolumeThreshold);
        ...
        micInputStream.on('sound', ({ volume }) => {
            timer.reset();
            process.stdout.write(`  ${chalk.green('===')}  ${chalk.green(volume)}                   \r`);
        });

        micInputStream.on('silence', ({ volume }) => {
            totalSilence++;
            process.stdout.write(`  ${chalk.dim.white('---')}  ${chalk.dim.white(volume)}                   \r`);
        });

        const output = fs.createWriteStream(filename);
        const writable = new Readable().wrap(micInputStream);
        writable.pipe(output);
        process.stdout.write(`  ${chalk.dim.white('---')}  0                   \r`);
        micInstance.start();
