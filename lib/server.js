...
import { getSynthAudio, googleSynth, mimicSynth, elevenSynth, toSSML } from './synth.js';
...
try {
    ...
    httpServer.get('/v1/message/:id', async (req, res) =>{
        ...
        let result;
        try {
            message = db.prepare(`select * from messages where id = @id`).get({ id });
            try {
            result = unpackMessage(message);
            } catch (error) {
            result = message;
            }
            res.send(result);
    ...
    httpServer.post('/v1/synth/:provider', async (req, res) => {
        ...
        const options = { ssml, voice };
        try {
            const audio = await getSynthAudio(provider, content, options);
            res.send(200, audio);
        } catch (error) {
            console.log(error);
            res.sendStatus(400);
    ...
    httpServer.post('/v1/ss-synth/google', async (req, res) => {
        ...
        const options = { ssml, voice };
        try {
            await googleSynth(content, options);
            res.sendStatus(200);
        } catch (error) {
            console.log(error);
            res.sendStatus(400);
    ...
    httpServer.post('/v1/ss-synth/eleven', async (req, res) => {
        console.log('POST - /v1/ss-synth/eleven');
        const content = req.body.ssml ?? req.body.content;
        const ssml = !!req.body.ssml;
        const options = { ssml };
        if (req.body.voice) options.voice = req.body.voice;
        try {
            await elevenSynth(content, options);
            res.sendStatus(200);
        } catch (error) {
            console.log(error);
            res.sendStatus(400);
